  ...
  setupLoaders() {
    ...
    this.postsByAuthorLoader = new DataLoader(async (authorIds) => {
      ...
      return authorIds.map(authorId => 
        postsByAuthor.filter(post => post.authorId === authorId)
      )
    })
    ...
    // Like status loader
    this.likeStatusLoader = new DataLoader(async (keys) => {
      if (!this.user) {
  ...
  Mutation: {
    register: async (parent, { input }, context) => {
      ...
      if (!validateUsername(input.username)) {
        throw new UserInputError('Username must be 3-20 characters, alphanumeric and underscore only')
      }
    ...
    login: async (parent, { email, password }, context) => {
      ...
      const isValid = await context.dataSources.userAPI.validatePassword(
        user.id, 
        password
      )
