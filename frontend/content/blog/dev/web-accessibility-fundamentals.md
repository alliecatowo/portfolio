  ...
  <fieldset>
    ...
    <div class="form-group">
      ...
      <input 
        ...
        aria-invalid="false"
      />
      <div id="fullName-help" class="form-help">
...
h1, h2, h3, h4, h5, h6, p, div:not([tabindex]) {
  outline: none;
}
```
  ...
  handleKeyDown(e) {
    ...
    switch (e.key) {
      ...
      case 'PageDown':
        e.preventDefault()
        newIndex = Math.min(
          this.currentIndex + this.columns * 5, 
          this.items.length - 1
        )
        break
    ...
    switch (direction) {
      case 'right':
        if (col < this.columns - 1 && this.currentIndex + 1 < this.items.length) {
          ...
          // Wrap to next row
          newIndex = row * this.columns + this.columns < this.items.length
