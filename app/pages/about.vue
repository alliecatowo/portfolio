...
const { data: aboutContent } = await useAsyncData('about-page-content', () => fetchPage('about'))

const page = computed(() => aboutContent.value ?? null)
const seo = computed(() => page.value?.seo)
const hero = computed(() => page.value?.hero ?? null)
const heroStats = computed(() => hero.value?.stats ?? [])
const heroParagraphs = computed(() => hero.value?.paragraphs ?? [])
const heroButtons = computed(() => hero.value?.buttons ?? [])
const journey = computed(() => page.value?.journey ?? null)
const journeyItems = computed(() => journey.value?.items ?? [])
const skills = computed(() => page.value?.skills ?? null)
const skillCategories = computed(() => skills.value?.categories ?? [])
const life = computed(() => page.value?.life ?? null)
const lifeCards = computed(() => life.value?.cards ?? [])
const cta = computed(() => page.value?.cta ?? null)
const ctaButtons = computed(() => cta.value?.buttons ?? [])
const ctaSocials = computed(() => cta.value?.socials ?? [])

const timelineStyles: Record<string, { text: string; bullet: string; card: string }> = {
...
const skillStyles: Record<string, { text: string; iconBg: string; gradient: string }> = {
  default: {
    text: 'text-primary',
...
const timelineTextClass = (color?: string) =>
  (color && timelineStyles[color]?.text) || timelineStyles.default.text

const timelineBulletClass = (color?: string) =>
  (color && timelineStyles[color]?.bullet) || timelineStyles.default.bullet

const timelineCardClass = (color?: string) =>
  (color && timelineStyles[color]?.card) || timelineStyles.default.card

const skillTextClass = (color?: string) =>
  (color && skillStyles[color]?.text) || skillStyles.default.text

const skillIconBackgroundClass = (color?: string) =>
  (color && skillStyles[color]?.iconBg) || skillStyles.default.iconBg

const skillGradientClass = (color?: string) => {
  const style = (color && skillStyles[color]) || skillStyles.default
  return `bg-gradient-to-r ${style.gradient}`
}

const lifeTextClass = (color?: string) =>
  (color && lifeStyles[color]) || lifeStyles.default
