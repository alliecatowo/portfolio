...
// Preload on hover
;<Link to="/dashboard" onMouseEnter={() => preloadRoute(() => import('./Dashboard'))}>
  Dashboard
</Link>
```
...
class DatabaseManager {
  ...
  async batchInsert(table, records) {
    ...
    const sql = `INSERT INTO ${table} (${keys.join(',')}) VALUES ${records
      .map(() => `(${placeholders})`)
      .join(',')}`
...
app.get('/api/large-dataset', async (req, res) => {
  ...
  query.pipe(jsonStream).pipe(gzipStream).pipe(res)
})
