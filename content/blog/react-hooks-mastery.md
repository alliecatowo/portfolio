...
const ProductList = ({ products, filters, sortBy }) => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    return products
      .filter(product => {
        return Object.entries(filters).every(([key, value]) => !value || product[key] === value)
      })
      .sort((a, b) => {
        switch (sortBy) {
          case 'price':
...
const ContactForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <FormField name="name" placeholder="Your name" formState={formState} />
      <FormField
        name="email"
        type="email"
        ...
        formState={formState}
      />
      <FormField name="message" as="textarea" placeholder="Your message..." formState={formState} />
